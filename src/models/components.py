...
class STNBlock(nn.Module):
    ...
    def __init__(self, in_channels: int = 3, dropout: float = 0.0):
        ...
        # Regressor for the 3x2 affine matrix (structure fixed for pretrained loading)
...
class AttentionFusion(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Weighted sum fusion + optional dropout
        fused_features = torch.sum(x_view * weights, dim=1)
        if self.dropout > 0 and self.training:
            fused_features = F.dropout(
                fused_features, p=self.dropout, training=True)
    ...
    def __init__(self, out_channels=512):
        ...
        self.features = nn.Sequential(
            ...
            nn.Conv2d(256, 256, 3, 1, 1), nn.ReLU(
                True), nn.MaxPool2d((2, 2), (2, 1), (0, 1)),
            # Block 4
            nn.Conv2d(256, 512, 3, 1, 1), nn.BatchNorm2d(512), nn.ReLU(True),
            nn.Conv2d(512, 512, 3, 1, 1), nn.ReLU(
                True), nn.MaxPool2d((2, 2), (2, 1), (0, 1)),
            # Block 5 (Map to sequence height 1)
            nn.Conv2d(512, out_channels, 2, 1, 0), nn.BatchNorm2d(
